...
# replace previous uniprot gene IDs with the new ensembl gene IDs
Celegans.PhyloMap.ENSEMBL <- phylomapr::Caenorhabditis_elegans.PhyloMap %>%
  dplyr::left_join(ENSEMBL_to_UNIPROT) %>%
  tidyr::drop_na()

Celegans.PhyloMap.ENSEMBL <- Celegans.PhyloMap.ENSEMBL %>%
  dplyr::select(-GeneID) %>%
